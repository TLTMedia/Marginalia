...
class UnapprovedComments 
{
    ...
    public function registerUnapprovedComment($ancestorEppn, $ancestorHash, $commenterEppn, $commentHash, $commentPath)
    {
        ...
        return $this->putContent($filename, json_encode($newUnapprovedComment));
    }
     ...
     * @return bool
     */
    private function putContent($filename, $data)
    {
        if (!$this->createIfNotExistsUnapprovedDir()) {
            // check log for why
            return FALSE;
        }
     ...
     */
    private function getUnapprovedCommentData($filename)
    {
        $fh = fopen($filename, "r");
        if (!$fh) {
            $this->logger->error(
     ...
     * @return bool
     */
    private function createIfNotExistsUnapprovedDir()
    {
