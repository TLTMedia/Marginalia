export class ReplyBoxController {
    ...
    createToolBar(toolbar_data) {
        ...
        let editButton = $("<button/>", {
            ...
            commentId: toolbar_data["hash"],
            click: (evt) => {
                this.editButtonOnClick(evt, toolbar_data["inText"], toolbar_data["hash"]);
            }
        });
    ...
    createMenuForComment(toolbar_data) {
        ...
        var menuDelete = $("<li/>", {
            ...
            commentid: toolbar_data["hash"],
            click: () => {
                this.deleteButtonOnClick(toolbar_data["hash"], toolbar_data["eppn"], toolbar_data["work"], toolbar_data["workCreator"]);
            }
        });
        var menuSetPrivate = $("<li/>", {
            ...
            commentid: toolbar_data["hash"],
            click: (evt) => {
                this.commentPrivateButtonOnClick(evt, toolbar_data["work"], toolbar_data["workCreator"], false);
            }
        });
        var menuSetPublic = $("<li/>", {
            ...
            commentid: toolbar_data["hash"],
            click: (evt) => {
                this.commentPrivateButtonOnClick(evt, toolbar_data["work"], toolbar_data["workCreator"], true);
            }
        });
        var menuApproveOrUnapprove = $("<li/>", {
            ...
            commentid: toolbar_data["hash"],
            click: (evt) => {
                this.commentApprovedOrUnapprovedButtonOnClick(toolbar_data["hash"], toolbar_data["eppn"], toolbar_data["work"], toolbar_data["workCreator"], toolbar_data["approved"]);
            }
        });
    ...
    async commentApprovedOrUnapprovedButtonOnClick(hash, commenterEppn, work, workCreator, approved) {
        var data = {
            creator: workCreator,
            work: work,
        ...
        let response = await this.state.api_data.comments_data.change_comment_approval(data, endpoint);
        ...
        this.refreshReplyBox(workCreator, work, firstCommenter, firstCommentHash);
    }
    ...
    async commentPrivateButtonOnClick(evt, work, workCreator, setPublic) {
        ...
        let response = await this.state.api_data.comments_data.change_comment_privacy(data);
        if (setPublic) {
        ...
        let firstCommentCommenter = this.state.replyBox_data.first_comment_author;
        this.refreshReplyBox(workCreator, work, firstCommentCommenter, firstCommentId);
    }

    async deleteButtonOnClick(hash, eppn, work, workCreator) {
        this.state.replyBox_data["delete_comment_id"] = hash;
        let first_comment_id = this.state.replyBox_data.first_comment_id;
        let first_comment_author = this.state.replyBox_data.first_comment_author;
        ...
        if (first_comment_id == this.state.replyBox_data.delete_comment_id) {
            ...
            this.refreshReplyBox(workCreator, work, first_comment_author, first_comment_id);
        }
    ...
    async refreshReplyBox(creator, work, commenter, hash, type) {
        ...
        let resp = await this.state.api_data.comments_data.get_comment_chain(comment_data);
        readThreads(resp);
    }
