...
function saveCommentOrReply(dataForSave,isFirstComment){
  ...
  API.request({
    ...
    // the first comment
      ...
      $("<param/>",{class : 'startDiv', commentId:data['commentHash'], startIndex:index["start"], isBlue:false}).insertBefore('.'+escapeSpecialChar(remSpan));
      $("<param/>",{class : 'endDiv', commentId : data['commentHash'], endIndex : index["end"], isBlue : false }).insertAfter('.'+escapeSpecialChar(remSpan));
...
function checkSpansNeedRecover(id,callback){
  ...
  else if(nextStartDivIndex < currentEndDivIndex && nextStartDivIndex != undefined){
  ...
  else if (prevEndDivIndex > currentStartDivIndex && prevEndDivIndex != undefined){
    removeDeletedSpan(id);
